...
%TALLENNETAAN
results = zeros((tend-t)/dt, 29);
results(1,:) = [t rk(1) rk(2) rk(3) rk(4) rk(5) rk(6) vk(1) vk(2) vk(3) vk(4) vk(5) vk(6) ...
    ak(1) ak(2) ak(3) ak(4) ak(5) ak(6) 0 0 0 0 0 0 0 0 meanh 0];
counter = 2;
...
while rk(1) - ship.rcog(1) < 150
    ...
    a_k = 6/dt^2*rk + 6/dt*vk + 2*ak;
    b_k = 3/dt*rk + 2*vk + 0.5*ak*dt;
    %initial values for position and speed, constant acceleration
    %assumed
    psi0 = rk(3)+vk(3)*dt+ak(3)*dt^2;
    TM = eye(6);%[cos(psi0) -sin(psi0) 0; sin(psi0) cos(psi0) 0; 0 0 1];
    ...
    xc = 0.372*vi0(1)^2/g;
    k = g/vi0(1)^2;
    [F_rotate,iceField_t] = rotateForces(iceField_t, ship, vi0,rhoi,rhow,dt,mu,t, Hbw, xc, k);
%     F_rotate = [F_rotate(1),0,0,0,0,0]';
    if ~isempty(Thrust)
        Ft = Thrust(round(t)+1);
    else
        power_t = power(1);
        pp_D = (length(prop_pitch)==1)*0+(length(prop_pitch)~=1)*prop_pitch(1);
        Ft = propellerForce(vi0, ship, pp_D, power_t,70);
    end
...
%     
%     iteration = 0;
%     %iterate within time step
%     while err>tol && iteration<1
%         %first iterate
%         ri = (6/dt^2*(M+A) + 3/dt*vi0(1)*BM)^-1*(F0 + (M+A)*a_k + vi0(1)*BM*b_k);
%         vi = vk + 0.5*ak*dt + 0.5*ai*dt;
%         [Fii, iceField_t, nLoad] = iceForces(ri, vi, ship, iceField, [0 0], t, lin,DB,areacorr);
% %         [F_rotate,iceField_t] = rotateForces(iceField_t, ship, vi,rhoi,rhow,hi,dt,mu,t);
% F_rotate = 0;
%         if isempty(Thrust)
%             Ft = propellerForce(vi0, ship, pp_D, power_t,70);
%         end
%         
%         Res = Rsub * (1 + 9.4 * norm(vi(1:2))/sqrt(9.81*ship.L))+Rridge;
%         Fxs = -Res * vi(1)/sqrt((vi(1))^2+(vi(2))^2);
%         Fxy = -Res * vi(2)/sqrt((vi(1))^2+(vi(2))^2);
%         Rs = [Fxs Fxy 0 0 0 0]';
%         Fboy = [0 0 -1025*9.8*ri(3)*ship.L*ship.B*0.8 -ship.GMroll*ri(4)*ship.M(1,1)*9.8 -ship.GMpitch*ri(5)*ship.M(1,1)*9.8 0]';
% %         Fi = Fii + Ft + Rs + Fboy + F_rotate;
%         Fi = Fii + Ft + Rs + Fboy ;
%         if sum(isnan(Fi))>0
%             error('Fi is nan')
%         end
% % Fi = Fii + Ft + Rs + Fboy;
%         ai = (M+A)^-1*(Fi-(vi0(1)*BM)*vi);
%         
%         err = norm(Fi-F0)/norm(F0);
%         F0 = Fi;
...
%     end
    ...
    if floor(5*t)-floor(5*(t-dt))==1
        ...
        if isfield(iceField.icepiece,'loc')
            scatter(iceField.icepiece.loc(:,1),iceField.icepiece.loc(:,2))
        end
        pause(0.001)
        hold off
    end
    
    results(counter,:) = [t rk(1) rk(2) rk(3) rk(4) rk(5) rk(6) vk(1) vk(2) vk(3) vk(4) vk(5) vk(6) ak(1) ak(2) ak(3) ak(4) ak(5) ak(6)...
        Fii(1) Fii(2) Fii(3) Fii(4) Fii(5) Fii(6) Fxs Ft(1) F_rotate(1) Rridge];
    ...
    function [Frotate, iceField] = rotateForces(iceField, ship, v,rhoi,rhow,dt,mu,t, Hbw, xc, k)
    ...
    %                 d2phi_over_dt2 = v(1)*interp1(ship.x(length(ship.x)/2+1:end),ship.d2phi_over_dx2(length(ship.x)/2+1:end),icepiece.loc(i,1));
                    vrel = [v(1)-v(6).*midy+v(5).*ship.zarm v(2)+v(6).*midx-v(4).*ship.zarm v(3)+v(4).*midy-v(5).*midx];
...
%                         del(j) = i; 
                        if icepiece.RelDis(i)==0
                            Fn(i) = icepiece.clength(i)*icepiece.h(i)*iceField.meansc;
                        else
                            Fn(i) = 0;
                        end
                    else

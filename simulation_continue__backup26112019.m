function [results, ship, iceField, normalLoads ] = ...
    simulation_continue(iceField,initial,tspan,ship,dt,power,Thrust,prop_pitch,lin,DB,vl,vr,icecondition,operationmode,scale,areacorr)
warning('off','all')
...
ship.rcog = initial(2:7)';
ship.vcog = initial(8:13)';
ship.F0 = initial(20:25)';
...
results = zeros(round((tend-t)/dt), 29);
results(1,:) = [t rk(1) rk(2) rk(3) rk(4) rk(5) rk(6) vk(1) vk(2) vk(3) vk(4) vk(5) vk(6) ak(1) ak(2) ak(3) ak(4) ak(5) ak(6) 0 0 0 0 0 0 0 0 meanh 0];
counter = 2;
...
while t < tend-0.0001 && rk(1) + ship.L < length(iceField.h(1,:))+ship.L*3
    ...
    v_action = [vi0(1),vi0(2)+vi0(6)*0.25*ship.L];
    ...
    Rs = [Fxs Fxy Fxz Fxy*0.5*ship.T -Fxz*0.25*ship.L-Fxs*0.5*ship.T Fxy*0.25*ship.L]';
    %rudder force
    ...
    F_nonlin = [M(1,1)*vi0(2)*vi0(6),-M(2,2)*vi0(1)*vi0(6),0,0,0,0]';
...
%     end
    ...
    if floor(50*t)-floor(50*(t-dt))==1
        ...
        vk(6)
        ri(6)
        F_rot_acc = F_rot_acc+F_rotate(6)
        hold on
    ...
    results(counter,:) = [t rk(1) rk(2) rk(3) rk(4) rk(5) rk(6) vk(1) vk(2) vk(3) vk(4) vk(5) vk(6) ...
        ak(1) ak(2) ak(3) ak(4) ak(5) ak(6) Fii(1) Fii(2) Fii(3) Fii(4) Fii(5) Fii(6) Fxs Ft(1) F_rotate(1) Fr(1)];

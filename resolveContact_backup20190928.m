function [ iceForce, iceField, nLoad ] = resolveContact( r, v, ship, iceField, reallyin, vchan, ~, lin, DB)
...
%                r(4)
        ...
        indforv = midy>0;
        vrel = vrel + [zeros(length(vrel(:,1)),1) indforv*vchan(1)-(1-indforv)*vchan(2) zeros(length(vrel(:,1)),1)];
...
%                     depth(j) = (0.5*depth1*depth2/pi)^0.5;
                    ...
                    depth(j) = isreal(the(j))*(abs(a(j)*ctrx+b(j)*ctry+c(j))/sqrt(a(j)^2+b(j)^2))/LC_DB(j);
                end
            else
                depth(j) = isreal(the(j))*(abs(a(j)*ctrx+b(j)*ctry+c(j))/sqrt(a(j)^2+b(j)^2))/LC_DB(j);
            end
...
%                 error('d')
%             end
            if meanPhi(j)<80 & depth(j)>0
                [Corr_v(j), Corr_A(j), Corr_ind(j)] = areacorrec(depth(j),hi(j),sigmac(j),meanPhi(j)/pi*180,iceField.E);
            else
                Corr_v(j) = 1;
                Corr_A(j) = 1;
                Corr_ind(j) = 1;
            end
            area(j) = area(j)*Corr_A(j);
            depth(j) = depth(j)*Corr_ind(j);
...
%                         center = 0.27/0.94*randomNumber*center+center;
                  ...
                  if angleInd >13
                      ...
                      rbreak_ind = find(db(thetaInd).stress(angleInd,:)*SF(i)+db(thetaInd).stressH(angleInd,:)*SF_H(i)<sigmaf(i));
...
%                     if isempty(sep_plus) | isempty(sep_minus)
%                         pause(0.1)
%                     end
                    if yint(2*i-1)<0
                    ...
                    b_wedge(i) = abs(abs(mean([y_sep1,y_sep2]))-abs(yint(2*i-1)));
                    c_wedge(i) = diff(xintsort);
                    %h and the were calculated previously
                    scale_midship = 1; % needs correction
                  %  fprintf("angle=%f,b=%f,c=%f,h=%f,mu=%f\n",the(i),b_wedge(i),c_wedge(i),hi(i),abs(FV(i)/FH(i)))
                    if b_wedge(i)>10
                        pause(0.1)
                    end
                    sigma_midship = 1e6*NN10neuron_mod([the(i)/180*3.14,b_wedge(i)*1000,c_wedge(i)*500,hi(i)*1000,abs(FV(i)/FH(i))]);
                    sigma_midship = 1e6*NN10neuron_mod([the(i)/180*3.14,b_wedge(i)*1000,c_wedge(i)*500,hi(i)*1000,0.1]);
                    sigma_midship = (sigma_midship>0)*sigma_midship;
                    sigma_midship = iceField.meansf + 1;
                    R_midship = NN10neuron_r([the(i)/180*3.14,b_wedge(i)*1000,c_wedge(i)*500,hi(i)*1000,abs(FV(i)/FH(i))])/1000;
                    R_midship = (R_midship>0)*R_midship;
                    R_midship = 0.3*l_c(i);
%                     plot(iceField.x,iceField.y,ship.x,ship.y)
                    sigma_midship = sigma_midship*scale_midship;
                    
                    if sigma_midship>iceField.meansf
                        nice = length(iceField.X);
                        dst = ((midX(i)*ones(1, nice) - iceField.X).^2 + (midY(i)*ones(1, nice) - iceField.Y).^2).^0.5;
                        cls = find(dst==min(dst), 1);%index of closest point
                        ...
                        cracksx = midX(i)+R_midship*cos(th);
                        cracksy = midY(i)+R_midship*sin(th);
...
%         if jjj ~= 1
        ...
        nLoad = [midx(:) midy(:) cLength(:) Fcr(:) FH(:) FV(:) fH(:) fV(:) LC_DB(:) sigmac(:) sigmaf(:) hi(:) RR(:) Redge(:) the(:) depth(:)];
%         else
%             Fx = 0;
%             Fy = 0;
%             Mz = 0;
